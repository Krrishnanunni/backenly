/**
 * AWS RDS certificate authorities for ap-south-1, embedded so the lineage probe
 * can verify the staging database certificate with no filesystem or network
 * dependency inside the task.
 *
 * Source   https://truststore.pki.rds.amazonaws.com/ap-south-1/ap-south-1-bundle.pem
 * Fetched  2026-09-15
 * Roots    Amazon RDS ap-south-1 Root CA RSA2048 G1 (valid to May 19 18:40:34 2061 GMT)
 *          Amazon RDS ap-south-1 Root CA ECC384 G1 (valid to May 19 18:50:59 2121 GMT)
 *          Amazon RDS ap-south-1 Root CA RSA4096 G1 (valid to May 19 18:45:20 2121 GMT)
 *
 * Public certificates, not secrets. Pinned so an edit is noticed rather than
 * silently trusted: loadRdsCa() refuses a bundle whose SHA-256 differs.
 */

import { createHash } from 'node:crypto'

export const RDS_CA_PIN_SHA256_BASE64 = 'ykqdwU4Gw/hCdO/z/+0OXU00YxQVk+EVnrSgkE32zXQ='

const RDS_CA_AP_SOUTH_1_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

t2/ewfi/0VhkeUW+IiHhOMdU
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`

export function loadRdsCa(): string {
  const actual = createHash('sha256').update(RDS_CA_AP_SOUTH_1_PEM).digest('base64')
  if (actual !== RDS_CA_PIN_SHA256_BASE64) {
    throw new Error('embedded RDS CA bundle does not match its pin; refusing to trust it')
  }
  return RDS_CA_AP_SOUTH_1_PEM
}
